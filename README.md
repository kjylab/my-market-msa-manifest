...
appConfig:                   # → ConfigMap으로 마운트 → Spring Boot 설정
  ...
  management:
    endpoints:
      web:
        base-path: /         # actuator base path (/ 로 설정)
        exposure:
...
ServiceMonitor (monitoring 네임스페이스)
  └─ "default 네임스페이스의 app=<release-name> 서비스를
      http 포트의 /prometheus 를 15초마다 스크래핑해라"
         ↓
...
> `management.endpoints.web.base-path: /` 설정으로 메트릭 경로가 `/actuator/prometheus`가 아닌 `/prometheus`임에 주의.
> ServiceMonitor의 `path` 설정도 `/prometheus`로 맞춰져 있어야 한다.

## Service 레이블 주의사항

ServiceMonitor가 서비스를 찾을 때 `app: {{ .Release.Name }}` 레이블을 사용한다.
`service.yaml`에 해당 레이블이 반드시 포함되어야 Prometheus 타겟이 정상 등록된다.
